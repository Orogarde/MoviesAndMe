...
class FilmDetail extends React.Component {
  ...
  _displayNoteButtons() {
    let views = [];
    let note = 0;
    const filmNote = this.props.noteFilms.find(value => value.film.id === this.state.film.id);
    if (!!filmNote) {
      note = filmNote.note;
    }
  ...
  _toggleNote(index) {
    const action = { type: "TOGGLE_RATED", value: {film: this.state.film, note: index}};
    this.props.dispatch(action);
